/***************************************************************************
 *                                                                         *
 *   Copyright (C) 2014 by Kinnective LLC                                   *
 *                                                                         *
...
$(document).ready(function(e) {
    ...
    touchScroll();
	$("#username").detach().prependTo('#useridDiv');
	$('#userid').remove();
	$('#password').detach().prependTo("#passwordDiv");
	$('#userpassword').remove();
	//$('#username').textinput();
	//$('#password').textinput();
	if(isKindle){
...
function attachHandlers(){
	$('#oursListContainerCG,#yoursListContainerCG').on('touchmousedown','.dataBarTitle',function(e) {
             //debugLog((downCounter++);
            e.preventDefault();
           ...
           itemThis.time=e.originalEvent.timeStamp;
            ...
            scrolling=true;
            diff=0;
            thisScroller = $(this).closest('.listBoxInner')[0];
            scrollToY=$(thisScroller).scrollTop()+e.pageY;
     });
    $("#oursListContainerCG,#yoursListContainerCG").on("touchmousemove",".dataBarTitle",function(e){
		e.preventDefault();
        if(scrolling){
                    postMouse(e);
					var a = thisScroller;
					...
					var thisS = thisScroller;
                    thisScroller.scrollTop=scrollToY-e.pageY;
                    }
        }
    });
    $("#oursListContainerCG,#yoursListContainerCG").on('touchmouseup',".dataBarTitle",function(e){
                  e.preventDefault();
                  scrolling=false;
				  //debugLog(("mouseup");
                 if(mouseLog.length > 1){
                     ...
                     var oldY=mouseLog[mouseLog.length-3].y;
                     var newTime=mouseLog[mouseLog.length-1].time;
                     var oldTime=mouseLog[mouseLog.length-3].time;
      ...
      $('#oursListContainer').on('click','.arrowRBtn',function(e) {
		             var target="ours";
                     e.preventDefault();
               ...
               $('#oursListContainer').on('click','.swoopBtn',function(ee) {
               });
               $('#oursListContainer').on('click','input.check',function(e) {
                        var item={};
						var target="ours";
                        item.wpID=waypID;
                        var barVar=$(this).parents('.dataBar')[0];
			   ...
			   $('#oursListContainer').on("rollover",'.grabBoxDiv',function(e) {e.stopImmediatePropagation();return false});
               $('#oursListContainer').on("touchmousedown",".grabBoxDiv", function(e) {
			   e.preventDefault();
			   e.stopImmediatePropagation;
			   var target = "ours";
			   var pageString="#ours";
			   var constring = "#oursListContainer";
               if(momentum!=null){
                    clearInterval(momentum);
                    momentum=null;
                }
               ...
               var thisGrab = $(this);
			   $(thisGrab).css('background-color','#f1c40f');
      ...
      $('#yoursListContainer').on('click','.arrowRBtn',function(e) {
		             var target="yours";
                     e.preventDefault();
                     //createAlertBox("listContainerCG .arrowBtn function fires");
                     var barVar=$(this).parents('.dataBar')[0];
               ...
               $('#yoursListContainer').on('click','.swoopBtn',function(ee) {
               });
               $('#yoursListContainer').on('click','input.check',function(e) {
				        var target="yours";
                        var item={};
                        item.wpID=waypID;
                        var barVar=$(this).parents('.dataBar')[0];
			   ...
			   $('#yoursListContainer').on("rollover",'.grabBoxDiv',function(e) {e.stopImmediatePropagation();return false});
               $('#yoursListContainer').on("touchmousedown",'.grabBoxDiv',function(e) {
			   e.preventDefault();
			   e.stopImmediatePropagation;
			   var target = "yours";
			   var pageString="#yours";
			   var constring = "#yoursListContainer";
               if(momentum!=null){
                    clearInterval(momentum);
                    momentum=null;
                }
     ...
     $('body').on('oursTrigger', function(e){
        ...
        oursBtns();
            ...
            createAlertBox("Your are not signed in. Please sign in or register on Sign In page to get credentials.")
        }
     });
     $('body').on('yoursTrigger', function(e){
        $.mobile.changePage("#yours");
        if(yoursWaypointID > -1){
        var dat={};
     ...
     $('#commentIn').keydown(function(e){
	     ...
	     $('#add').on('pageshow', function(e){
           ...
           if(urlParams.url){
                 $('#radioLink').prop("checked",true).checkboxradio("refresh");
                 $('#radioFolder').prop("checked",false).checkboxradio("refresh");
                 $('#linkInput').show();
                 $('#folderIn').val(urlParams.title);
                 $('#linkIn').val(urlParams.url);
           }else if(!urlParams.url){
                 $('#radioFolder').prop("checked",true).checkboxradio("refresh");
                 $('#radioLink').prop("checked",false).checkboxradio("refresh");
                 $('#folderIn').val("");
                 $('#linkIn').val("");
                 $('#linkInput').hide();
           }
     ...
     $('#yoursAdd').on('pageshow', function(e){
           ...
           if(urlParams.url){
                 $('#yoursRadioLink').prop("checked",true).checkboxradio("refresh");
                 $('#yoursRadioFolder').prop("checked",false).checkboxradio("refresh");
                 $('#yoursLinkInput').show();
                 $('#yoursFolderIn').val(urlParams.title);
                 $('#yoursLinkIn').val(urlParams.url);
           }else if(!urlParams.url){
                 $('#yoursRadioFolder').prop("checked",true).checkboxradio("refresh");
                 $('#yoursRadioLink').prop("checked",false).checkboxradio("refresh");
                 $('#yoursFolderIn').val("");
                 $('#yoursLinkIn').val("");
                 $('#yoursLinkInput').hide();
           }
      ...
      $('#searchAddBtn').click(function(e) {
         ...
         $('#oursDoneAddBtn').click(function(e) {
            if(adding == false){
               ...
               adding=true;
               $('.mainContainer').scrollTop(0);
               var val=$('#radioLink').prop("checked");
         ...
         $('#yoursDoneAddBtn').click(function(e) {
            if(adding == false){
              ...
              patience=0;
              $('.mainContainer').scrollTop(0);
              var val=$('#yoursRadioLink').prop("checked");
...
//dan
         ...
         $('#downloadBtn').click(function(e){
            upload=false;
			$('#upload .contentDiv').css('overflow','auto');
      ...
      $('#cancelSubmit').click(function(e) {
         //createAlertBox("signout function fires");
            ...
            $('#ours .arrowLBtn').click(function(e) {
			   ...
			   if(oursWaypointID < 0){
				   kinn.anamolyCall({error:"This is the minus one error.",OWPID:oursWaypointID,button:"ours_arrowLBtn"});
			   }
         ...
         $('#yours .arrowLBtn').click(function(e) {
			if(yoursWaypointID < 0){
				   kinn.anamolyCall({error:"This is the minus one error.",YWPID:oursWaypointID,button:"yours_arrowLBtn"});
			}
...
function layout(){
    ...
    //size= measure.offsetHeight/10;
    //document.body.removeChild(measure);
	var maxHeaderHeight=200;
	var minHeaderHeight=115;
	var maxFooterHeight=80;
	var minFooterHeight=43;
	var maxContainerWidth = 745;
	var navHeight = 40;
	var logoBottom =37;
	var titleConHeight = 65;
	var resultWiggle = 152;
	var headerContainerHeight=42;
	var setHeaderHeight;
	var setFooterHeight;
	var setContentHeightM;
	var setContentHeightNF;
	var setContentHeightNFNH;
	var setContainerWidth;
	var size;
    var calcHeaderHeight=winHeight*.15;
    var calcFooterHeight=winHeight*.06;
	$('.footerDiv a').css('font-size','14px');
	$('.footerDiv a').css('padding-top','4px');
	$('.footerDiv a').css('padding-bottom','4px');
	$('.footerDiv a').css('padding-right','6px');
	$('.footerDiv a').css('padding-left','6px');
	$('.footerDiv a').css('margin','2px');
	if(winWidth <= 360){
		minHeaderHeight=75;
		navHeight= 32;
		logoBottom = 28;
		titleConHeight = 50;
		resultWiggle = 137;
		$('.mainTitle p').css('font-size',"1.5em");
		$('.breadCrumbs p').css('font-size','1.1em');
		$('.headerContainer h1').css('font-size','1.5em');
		$('.footerDiv a').css('padding-top','3px');
		$('.footerDiv a').css('padding-bottom','3px');
		$('.footerDiv a').css('padding-right','5px');
		$('.footerDiv a').css('padding-left','5px');
		$('.footerDiv a').css('margin','2px');
		...
		$('.mainTitle p').css('font-size',"1.5em");
		$('.breadCrumbs p').css('font-size','1.1em');
		$('.headerContainer h1').css('font-size','1.7em');
		$('.footerDiv a').css('padding','5px');
	}
    if(calcHeaderHeight > maxHeaderHeight) {
        setHeaderHeight=maxHeaderHeight;
    }else if(calcHeaderHeight < minHeaderHeight){
        setHeaderHeight=minHeaderHeight;
    }else{
        setHeaderHeight=calcHeaderHeight;
    }
    if(calcFooterHeight > maxFooterHeight) {
        setFooterHeight = maxFooterHeight;
    }else if(calcFooterHeight < minFooterHeight){
        setFooterHeight = minFooterHeight;
    }else{
        setFooterHeight=calcFooterHeight;
    }
    var sumHeight = setFooterHeight + setHeaderHeight;
    setContentHeightM = winHeight - sumHeight;
	setContentHeightNF = winHeight - setHeaderHeight;
	setContentHeightNFNH = winHeight;
    if(winWidth >= 740){
        setContainerWidth = 740;
    }else{
        setContainerWidth = winWidth;
    }
	var logoHeight = setHeaderHeight - navHeight;
    $(".mainContainer").css('width',setContainerWidth-4);
    $(".mainContainer").css('height',winHeight-4);
	$(".regularContent").css("height",setContentHeightM);
	$(".biggerContent").css("height",setContentHeightNF);
	$(".biggestContent").css("height",setContentHeightNFNH);
	$(".headerDiv").css("height",setHeaderHeight);
	$(".mainNav").css('height',navHeight);
	$(".logo").css('height',logoHeight);
	$(".logo").css("bottom",logoBottom);
	$(".titleContainer").css("height",titleConHeight);
	$(".footerDiv").css("height",setFooterHeight-2);
	$('#searchAddBtn').css('background-color','#ccc');
	var listHeight;
	if(winWidth >360){
		listHeight=setContentHeightM-105;
	}else if(winWidth <= 360){
		listHeight=setContentHeightM-50;
	}else if(winWidth<=320){
		listHeight=setContentHeightM-30;
	}
	$('.listBox').css("height",listHeight);
	var setContentContainerHeight = winHeight - headerContainerHeight;
	$('.contentContainer').css('height',setContentContainerHeight);
	var resultsHeight = setContentContainerHeight - resultWiggle;
	$('#searchResultsContainer').css('height',resultsHeight);
	var searchListHeight = resultsHeight-80;
	$('#searchListContainer').css('height',searchListHeight);
	var copiesListHeight = setContentContainerHeight - 75;
	$('#copiesListContainer').css('height',copiesListHeight);
	var discussListHeight = winHeight -225;
	$('#discussListContainer').css('height',discussListHeight);
	scaleFonts();
	buildHeaders();
}
function getWidth()
...
function scaleFonts(){
	var percentage = winWidth/768;
	if(percentage < 1){
		var scaler = .50 + (.50 * percentage);
		var pxScaler = 16 * scaler;
		$('body').css("font-size","" + (pxScaler) + "px");
		$('a').css("font-size",11);
		$('.buttonContainer a').css('padding','6px');
		var logoScaler = 5 * scaler;
		$('.logoSpan').css("font-size","" + (logoScaler) + "em");
		$('#userN').css("font-size",12);
		$('#usePW').css("font-size",12);
	}
}
function buildHeaders(){
	if(winWidth >768 ){
		$('.fourBtns').remove();
	}else if(winWidth<=768  && parent.explorePage==true){
		$('.fourBtns').remove();
	}else{
		$(".fiveBtns").remove();
	}
...
function oursCopy(){
   ...
   function renderMainView(xmlInfo,target,com) {
               ...
               var topString1B='" id="';
               var topString1C='"><div class="grabBoxDiv"><a href="#" class="ui-shadow ui-mini ui-icon-grid upGrab ui-btn-icon-notext" data-role="button"></a></div><!-- end of grabBoxDiv--><div class="checkBoxDiv"><input class="check" type="checkbox" data-role="none" title="Select this Direction"></input></div><!-- end of checkBoxDiv --><div class="dataBarTitle"><p class="dataBarTitleP"><span class="titleSpan">';
               var topString2='</span></p></div><!-- end of dataBarTitle --><div class="swoop"><a href="';
               var topString3='" class="ui-btn swoopBtn ui-shadow ui-mini ui-icon-forward ui-btn-icon-notext"></a></div><!-- end of swoop -->';
               var topString4='<div class="arrowR"><a href="#" class="ui-btn arrowRBtn ui-shadow ui-mini ui-icon-carat-r ui-btn-icon-notext" title="Folder"</a></div><!-- end of arrowR -->';
               var topString5='</div><!-- end of dataBar -->';
               var barString1A='<div class="ui-bar dataBarColor dataBar" number="';
               var barString1B='" id="';
               var barString1C='"><div class="grabBoxDiv"><a href="#" class="ui-shadow ui-mini ui-icon-grid upGrab ui-btn-icon-notext" data-role="button"></a></div><!-- end of grabBoxDiv --><div class="checkBoxDiv" data-role="none"><input class="check" type="checkbox" data-role="none" title="Select this Direction"></input></div><!-- end of checkBoxDiv --><div class="dataBarTitle"><p class="dataBarTitleP"><span class="titleSpan">';
	 ...
	 if(parent.buildChart != undefined){
	 	parent.buildChart(nodeNumber,target);
	 }
   ...
   function renderCommentsView(comXML){
      ...
      var midString4='</p></div><div class="commentVote" data-role="controlgroup" data-type="horizontal" data-mini="true"><a href="#" class="ui-btn ui-shadow ui-icon-plus ui-btn-icon-left commentPlus">Agree</a><a href="#" class="ui-btn ui-shadow ui-icon-minus ui-btn-icon-left commentMinus">Disagree</a></div></div>';
   ...
   function renderSearchView(xmlInfo) {
              ...
              var urlVar;
               ...
               var topString1D='"><div class="checkBoxDiv"><input class="check" type="checkbox" title="Select this Direction"></input></div><!-- end of checkBoxDiv --><div class="dataBarTitle"><p class="dataBarTitleP"><span class="titleSpan">';
               var topString2='</span></p></div><!-- end of dataBarTitle --><div class="swoop"><a href="';
               var topString3='" class="swoopBtn ui-btn ui-shadow ui-mini ui-icon-forward ui-btn-icon-notext" title="Go to link">Link</a></div>'
               var topString4='<div class="arrowR"><a href="#" class="arrowRBtn ui-btn ui-shadow ui-mini ui-icon-carat-r ui-btn-icon-notext" title="Go to folder">Folder</a></div><!-- end of arrowR -->';
               var topString4B='<div class="searchArrowLDiv"><a href="#" class="searchArrowLBtn ui-btn ui-shadow ui-mini ui-icon-carat-l ui-btn-icon-notext" title="Go to parent Waypoint">Parent folder</a></div><!-- end of searchArrowL -->';
               var topString5='</div><!-- end of dataBar -->';
               ...
               $('#searchListContainerCG .dataBar').each(function () {
                  if($(this).attr("type") == 1 || $(this).attr("type") == 3){
                     if($(this).hasClass('dataBarColor')){
						...
						$(this).find('.swoopBtn').css('background-color','#ccc');
						$(this).find('.arrowRBtn').css('background-color','#ccc');
						$(this).find('.searchArrowLBtn').css('background-color','#ccc');
                     }
                  }
               });
...
function renderCopiesView(){
               ...
               var topString2='</span></p></div><!-- end of dataBarTitle --><div class="swoop"><a href="';
               var topString3='" class="swoopBtn ui-btn ui-shadow ui-mini ui-icon-forward ui-btn-icon-notext" title="Link">Link</a></div><!-- end of swoop -->';
               var topString4='<div class="arrowR"><a href="#" class="arrowRBtn ui-btn ui-shadow ui-mini ui-icon-carat-r ui-btn-icon-notext" title="Folder"</a></div><!-- end of arrowR -->';
               var topString5='</div><!-- end of dataBar -->';
               var barString1A='<div class="dataBarColor dataBar" number="';
               var barString1B='" id="';
   ...
   function sign(){
         if(parent.explorePage==true){
			$('#username').val("explore");
			$('#password').val("kinnective");
		 }
...
function postMouse(e){
  ...
  kinn.frameLoaded=function(){
         ...
         if(patt.test(e)){
            ...
            createAlertBox("Your favorites have loaded. You will now be redirected to the Private page.");
            var to=setTimeout(function(){
               getYours();
               },4000);
   ...
   function oursBtns(){
	  ...
	  $('.footerDiv a,.footerBtns a').css('background-clip','border-box');
   }
   function yoursBtns(){
      var ch=copiesCheckedModel.length;
      var cop=copiesModel.length;
   ...
   function createAlertBox(string){
      var alertBoxString='<div class="alertBox dataBarColor ui-corner-all"><div class="alertBtnDiv"><input CLASS="deleteAlert ui-btn ui-corner-all ui-shadow ui-mini ui-btn-icon-left ui-icon-delete" value="Done"/></div><div class="innerAlertBox"><span class="alertSpan"></span></div></div>';
      var dummyDiv=document.createElement('div');
	  //console.log("alertBox should close");
   ...
   function fourBtns(){
	...
	kinn.flip = function(node,target,pathnum){
		var params = {};
		params.command = 1;
		params.node = node;
		params.wp_direction = pathnum;
		if(target=="ours"){
			getOursAgain(params);
